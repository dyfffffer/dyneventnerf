...
public:
	...
	void setAudioParams(int channels, int sampleRate, int bitRate, OptValueMap&& valueMap) noexcept
	{
		...
		av_channel_layout_copy(&codecContext_->ch_layout, &layout);
...
private:
	...
	void setGenericDefaultValues() noexcept
	{
		switch (codecContext_->codec->type)
		{
			...
			// 替换原来的代码块
			if (codecContext_->codec_type == AVMEDIA_TYPE_AUDIO) {
				...
				// 如果有支持的声道布局列表，选择第一个
				if (codecContext_->codec->ch_layouts) {
					av_channel_layout_copy(&codecContext_->ch_layout, &codecContext_->codec->ch_layouts[0]);
					for (int i = 0; codecContext_->codec->ch_layouts[i].nb_channels; i++) {
						// if (av_channel_layout_compare(&codecContext_->codec->ch_layouts[i],
						// 							&(AVChannelLayout)AV_CHANNEL_LAYOUT_STEREO) == 0) {
						AVChannelLayout stereo = AV_CHANNEL_LAYOUT_STEREO;
						if (av_channel_layout_compare(&codecContext_->codec->ch_layouts[i],
													&stereo) == 0) {
							av_channel_layout_uninit(&stereo);
							av_channel_layout_copy(&codecContext_->ch_layout, 
												&codecContext_->codec->ch_layouts[i]);
							break;
						}
					}
				}
